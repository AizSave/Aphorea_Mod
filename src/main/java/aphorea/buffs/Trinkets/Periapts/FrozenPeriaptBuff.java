...
public class FrozenPeriaptBuff extends AphoreaPeriaptActivableBuff {

    public FrozenPeriaptBuff() {
        super("frozenperiaptactivebuff");
    }
    ...
    public void onHasAttacked(ActiveBuff buff, MobWasHitEvent event) {
        if(!event.wasPrevented && event.attacker.getAttackOwner().isPlayer) {
            ...
            if(event.damageType.equals(DamageTypeRegistry.MELEE) || event.damageType.equals(DamageTypeRegistry.RANGED) || owner.buffManager.hasBuff("frozenperiaptactivebuff")) {
                if(!event.target.buffManager.hasBuff(BuffRegistry.Debuffs.FREEZING)) {
