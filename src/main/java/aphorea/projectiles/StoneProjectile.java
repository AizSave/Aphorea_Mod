...
public class StoneProjectile extends Projectile {
    ...
    public StoneProjectile(Level level, Mob owner, float x, float y, float targetX, float targetY, float speed, int distance, GameDamage damage, int knockback) {
        this.setLevel(level);
        this.setOwner(owner);
        this.x = x;
        this.y = y;
    ...
    public Trail getTrail() {
        return new Trail(this, getLevel(), new Color(153, 153, 153), 26, 100, getHeight());
    }
    ...
    public void addHit(Mob target) {
        super.addHit(target);
        if(Math.random() < 0.5) {
            ActiveBuff buff = new ActiveBuff(BuffRegistry.getBuff("stunbuff"), target, 500, this);
