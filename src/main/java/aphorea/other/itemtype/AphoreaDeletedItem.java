...
abstract public class AphoreaDeletedItem extends AphoreaConsumableItem {
    public AphoreaDeletedItem() {
        super(1, true);
    ...
    public int cantidadLibre(PlayerMob player) {
    ...
    public String canPlace(Level level, int x, int y, PlayerMob player, InventoryItem item, PacketReader contentReader) {
        if(cantidadLibre(player) < items(player).size()) return "nofreeslots";
        return null;
    }
