...
import aphorea.projectiles.FrozenStoneProjectile;
...
public class FrozenSling extends AphoreaSlingToolItem {
    public FrozenSling() {
        super(100, "frozenstoneprojectile");
        this.rarity = Rarity.NORMAL;
        this.attackAnimTime.setBaseValue(1200);
        this.attackDamage.setBaseValue(40.0F).setUpgradedValue(1.0F, 150.0F);
    ...
    public Projectile getProjectile(Level level, int x, int y, PlayerMob player, InventoryItem item) {
        return new FrozenStoneProjectile(level, player,
                player.x, player.y,
                x, y,
